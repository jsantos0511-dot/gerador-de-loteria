...
TEMAS = {
    ...
    "Mega-Sena": {"cor": "#209869", "titulo": "Mega-Sena", "cols": 6, "total": 60},
    "Lotofácil": {"cor": "#930089", "titulo": "Lotofácil", "cols": 5, "total": 25},
    "Quina": {"cor": "#260085", "titulo": "Quina", "cols": 8, "total": 80},
    "Lotomania": {"cor": "#f7941d", "titulo": "Lotomania", "cols": 10, "total": 100}
}
...
st.markdown(f"""
    ...
    /* Cor do botão selecionado conforme o tema */
...
def gerador_loteria(nome, total):
    ...
    with c1:
        if st.button("🎲 Surpresinha", use_container_width=True):
            qtd_surpresa = 15 if nome == "Lotofácil" else 6
            st.session_state[key_sel] = [f"{i:02d}" for i in random.sample(range(1, total + 1), qtd_surpresa)]
    with c2:
    ...
    # Volante
    opcoes = [f"{i:02d}" for i in range(1, total + 1)]
    selecionados = st.segmented_control("Volante", options=opcoes, selection_mode="multi", key=key_sel, label_visibility="collapsed")
    
    st.write(f"**Selecionados:** {len(selecionados)} de {total}")
    st.divider()

    # --- INPUTS REINSTALADOS ---
    col_a, col_b = st.columns(2)
    with col_a:
        padrao_dez = 15 if nome == "Lotofácil" else 6
        dez_por_jogo = st.number_input("Bolas por jogo", padrao_dez, 20, padrao_dez)
        valor_unit = st.number_input("Preço da Aposta R$", 0.0, 5000.0, 5.0 if nome != "Lotofácil" else 3.0)
    with col_b:
            ...
            with st.spinner("Processando..."):
                ...
                if res:
                    ...
                    st.download_button("💾 Baixar Planilha", csv_io.getvalue().encode('utf-8-sig'), 
                                     f"jogos_{nome.lower()}.csv", "text/csv", use_container_width=True)
                else:
                    st.warning("Nenhum jogo atende aos filtros.")

# --- ROTEAMENTO ---
if menu == "Início":
    home()
else:
    gerador_loteria(menu, tema_atual["total"])
